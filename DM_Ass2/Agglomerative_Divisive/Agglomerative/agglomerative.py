...
class Cluster:
	...
	def generateInitialDistanceMatrix(cls, test = False):
		...
		# Actual Dataset Implementation
			...
			# pickleFilePath = Path('data1/simMat1_3.pkl')
	...
	def mergeSimilarClusters(cls, mergedRC, toDelete, iteration, dist, heuristic = 'Centroid'):
		...
		# print('Combining clusters by Union operation ({} - {}), distance {}'.format(toDelete, mergedRC, dist))
...
def main():
	...
	parser.add_argument("echo",help="Please give arguments as 'Centroid','Min','Max'")
...
def drawDendrogram(UniObj, labels, heuristic):
	...
	plt.savefig('data/results/{}'.format(heuristic))
	# plt.savefig('data1/results/{}'.format(heuristic))
